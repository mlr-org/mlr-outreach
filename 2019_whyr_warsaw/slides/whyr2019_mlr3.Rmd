...
.center[
Slides: [bit.ly/whyr2019_mlr3](https://raw.githack.com/mlr-org/mlr-outreach/master/2019_whyr_warsaw/slides/whyr2019_mlr3.html#1)  
Examples: [bit.ly/whyr2019_mlr3ex](https://raw.githubusercontent.com/mlr-org/mlr-outreach/master/2019_whyr_warsaw/exercise/mlr3_exercises.Rmd)
]
]
...
* Steeper learning curve to learn mlr3
  * Knowledge summarized in [`r icon::fa("book")` mlr3book](https://mlr3book.mlr-org.com/)
...
Performance of most methods depends on hyperparameters, choose them so the method performs well on the given data.
...
* In process:
  - _Bayesian Optimization_, _Iterated F-racing_, _EAs_, _Hyperband_
...
* How do I start?
  - Check these slides
  - Check the [`r icon::fa("book")` mlr3book](https://mlr3book.mlr-org.com/)
* What can I do with these R6 objects?
  ...
  ?BenchmarkResult
  ```
  - _Fields_ documents data stored in this object and _Methods_ documents functions of this object.
* Why does this not work?
  - Ask at [`r icon::fa("stack-overflow")` stackoverflow](https://stackoverflow.com/questions/tagged/mlr3).
...
So far *mlr3* just modeled this part (model fitting):
<div style="
    ...
    left: 324px;
    top: 416px;"></div>
```{r fig_pipelines, echo = FALSE, out.width="70%"}
```

mlr3pipelines creates ML workflows for
- preprocessing (feature extraction/selection, missing data imputation etc.).  
- ensemble models (model averaging, model stacking)

---

# Preprocessing Pipelines

We can put preprocessing operations (*PipeOps*) in front of a Learner
...
Like learners, *PipeOps* have hyperparameters controlling their behavior.
...
`r icon::fa("info-circle")` Remember: Transforming the data before cross-validating skews results. Transforming the data independently for training and test leads to wrong results!
...
.pull-left[
Access *PipeOp* hyperparameter through parameter with its prefix in the *GraphLearner*.
...
Check the [`r icon::fa("book")` mlr3book paradox chapter](https://mlr3book.mlr-org.com/paradox.html).
]
...
`r icon::fa("info-circle")` Check the [`r icon::fa("book")` mlr3book pipelines chapter](https://mlr3book.mlr-org.com/pipelines.html) for exhaustive explanations and more examples.
...
* Inexpensive to work on:
  ...
  * `[i, ]`: look-up row (possibly hashed), create a list of pointers
...
Task stores data in a `DataBackend`:
  ...
  * `DataBackendCbind`: Combine backends through `task$cbind(backend)` (virtual)
  
  * `DataBackendRbind`: Combine backends through `task$rbind(backend)` (virtual)
...
- Under active development
  ...
  - GH versions of packages can be incompatible at short periods
  - UX will be improved (feedback welcome!)
- On CRAN
  - mlr3, mlr3db, mlr3filters, mlr3learners
- On GitHub
  - mlr3pipelines, mlr3survival, mlr3tuning, mlr3viz, mlr3spatiotemporal
- [More coming](https://github.com/mlr-org/mlr3/wiki/Extension-Packages)!
- [`r icon::fa("book")` mlr3book](https://mlr3book.mlr-org.com) covers actual state

Want to contribute?
[`r icon::fa("github")` mlr3.mlr-org.com](https://mlr3.mlr-org.com)
]

.pull-right[
<iframe width="320" height="180" src="https://www.youtube.com/embed/wsP2hiFnDQs" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
<iframe width="320" height="180" src="https://www.youtube.com/embed/gEW5RxkbQuQ" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
]
