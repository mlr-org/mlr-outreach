...
If you run this tutorial on your laptop, you should make sure all packages used in this tutorial are installed. This includes packages from the `mlr3` family.

This is *not* necessary in rstudio cloud sessions that were prepared for this course!
...
- We fit a decision tree with depth 3 for nicer plots that are easier to interprete.
```{r}
learner_cart = lrn("classif.rpart", maxdepth = 3)
learner_cart$train(task)
```
- The `rpart.plot` library gives expressive plots for the splits:
```{r}
...
- We simulate new data by sampling from the credit data.
```{r, R.options=list(width = 120)}
# Just generate *some* prediction data for this demo!
...
pred_cart = learner_cart$predict_newdata(newdata)
...
- the resampling result can also be converted to `data.table`
```{r}
as.data.table(res)
```

- We can easily do differend resampling schemes, e.g. repeated holdout (`"subsampling"`), or cross validation.
- Most methods do repeated train/predict cycles on different data subsets and aggregate the result (usually as the `mean()`).
- If we were to do this manually we would have to write loops.
```{r}
...
- In General, we want use *parallelization* to speed things up on multicore machines. This breaks on rstudio cloud! Uncomment the line if you are running this locally.
```{r}
...
* What are possible argument values of `lrn()`, `tsk()`, etc. again? -> Think about the corresponding dictionary
```{r}
mlr_learners$keys()
