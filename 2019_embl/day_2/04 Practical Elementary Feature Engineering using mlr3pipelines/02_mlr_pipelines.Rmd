...
In this case we will continue working with the **German Credit Dataset** again. We already used different `Learner`s on it and tried to optimize their hyperparameters. Now we will
...
- We can impute numeric features by their median. If we had factorial features, we could also add an imputer for them, e.g. `po("imputenewlvl")`.
...
- If we used the imputed task for resampling, we would leak information from the test set into the trainingset. Therefore it is mandatory to attach the imputation operator to the `Learner` itself, creating a `GraphLearner`.
```{r}
...
- We use the imputed task here because `filter$calculate` can not handle missing data! When we use filtering in a pipeline, we will have to prefix the filter with `po("imputemean")`.
```{r}
library("mlr3filters")
mlr_filters
...
- If we plot this, we see the possible tradeoffs between sparsity and predictive performance.
```{r}
...
- When training with a small datasset or datasets with many factor levels it is possible that not all possible factor levels are visible to the `Learner`
```{r, error = TRUE}
