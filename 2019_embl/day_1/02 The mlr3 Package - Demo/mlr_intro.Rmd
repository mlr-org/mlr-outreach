...
DataExplorer::plot_correlation(credit, type = "continuous")
```
...
newdata
```
- We create a `Task` from this new data, so that the `Learner`'s `$predict()` function can be used.
```{r}
new_task = TaskClassif$new("new_credit", newdata, "class")
```
...
## Performance Evaluation: The Hard Way

To show what is happening behind the scenes we will do one round of "holdout" resampling ourselves:
1. Decide on a train/test split, i.e., which rows should be used for the training set and test set.
```{r}
...
```
2. Here, we use the `"classif.rpart"` learner, and fit a model only using the training set (the `row_ids` argument of the `$train()` function lets us train on a subset of the data).
```{r}
...
```{r}
# manual calculation of the misclassification rate
mean(prediction$truth != prediction$response)

# more scalable and less errorprone: Using a pre-defined measure with msr()
msr("classif.ce") # measure for the misclassification error
...
The most common measures are implemented in `mlr3`, other measures can be obtained from 

```{r}
mlr_measures
```

## Performance Evaluation: The Easy Way
