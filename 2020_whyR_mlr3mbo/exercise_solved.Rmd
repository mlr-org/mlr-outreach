...
tune_fun = function(xs) {
  ...
  res = resample(task, this_learner, resampling) #!
  res = res$aggregate(measures = measure) #!
  return(list(y=res))
}
```
...
```{r}
grid = expand.grid(cp = c(0.02, 0.05, 0.09), minsplit = c(1,5,7)) #!=
...
inst$eval_batch(as.data.table(grid)) #!
```
...
```{r}
inst$eval_batch(data.table(minsplit = 4, cp = 0.01)) #!(
...
```{r}
library(mlr3mbo)
surrogate = SurrogateSingleCritLearner$new(learner = lrn("regr.km", nugget.estim = TRUE, covtype = "matern3_2")) #!lrn(
...
acq_optimizer$param_set$values$iters = 1100 #!$iters
```
...
repeat {
  ...
  surrogate$update(xydt = xydt[, c(archive$cols_x, archive$cols_y), with = FALSE], y_cols = archive$cols_y) #update surrogate model with new data
  acq_function$update(archive) 
  acq_function$param_set$values$lambda = 1L #!
  xdt_1 = acq_optimizer$optimize(acq_function) #!
  acq_function$param_set$values$lambda = 10L #!
  xdt_10 = acq_optimizer$optimize(acq_function) #!
  xdt = rbind(xdt_1, xdt_10) #!
  instance$eval_batch(xdt)
  if (instance$is_terminated || instance$terminator$is_terminated(archive)) break
}
```
